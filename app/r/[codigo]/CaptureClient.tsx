...
import { CheckCircle, Phone, User, Stethoscope, Shield, ChevronDown, MessageCircle, RefreshCw } from "lucide-react"
...
// ── CAPTCHA matemático en canvas ──────────────────────────────────────────────

type Challenge = { a: number; b: number; op: string; result: number }

function CaptchaCanvas({ onVerified }: { onVerified: (ok: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    // Líneas onduladas de ruido
    for (let l = 0; l < 3; l++) {
      ctx.beginPath()
      ctx.strokeStyle = `rgba(99,102,241,${0.07 + Math.random() * 0.07})`
      ctx.lineWidth = 0.8
      ctx.moveTo(0, 12 + Math.random() * 32)
      for (let x = 20; x <= W; x += 20) ctx.lineTo(x, 10 + Math.random() * 36)
      ctx.stroke()
    }

    // Texto del reto con leve rotación por carácter
    ...
    Array.from(text).forEach((char) => {
      ...
      ctx.save()
      ctx.translate(cx, cy)
      ctx.rotate(Math.random() * 0.26 - 0.13)
    ...
    <div className="rounded-2xl border p-4 space-y-3"
      ...
      <div className="flex items-center gap-2">
        <canvas ref={canvasRef}
          style={{ borderRadius: 8, border: "1px solid #C7D2FE", flex: 1 }} />
      ...
      <div className="flex gap-2">
        <input
          ...
          style={{ ...inputBaseNoUpper, borderColor: captchaErr ? "var(--negative)" : "var(--border)", flex: 1 }}
        />
